<template lang='html'>
    <div class='multil-cascader'>
        <el_popover placement="top-start" popper-class="multi-cascader-popover" :visible-arrow="showArrow" trigger="click" @hide="whenPopoverHide" @show="whenPopoverShow">
            <muContent
                :height="height"
                :width="width"
                :option="options"
            ...
            <el_input popper-class="slect-panel" v-if="activeItem[0] && activeItem[0].level === 0"  v-model="inputValue" readonly slot="reference" :suffix-icon="inputArrow"/>
        </el_popover>
    </div>
</template>

<script>
import muContent from "./multiContent.vue";
export default {
    name: "multiCascader",
    props: {
        options: {
